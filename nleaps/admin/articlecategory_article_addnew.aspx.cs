...
{
    public partial class articlecategory_article_addnew : PageBase
    {
        #region ViewPower
        ...
        private void LoadData()
        {
            ...
            ArticleCategory current = DB.ArticleCategorys.Find(id);
            if (current == null)
            {
        ...
        protected void btnSaveClose_Click(object sender, EventArgs e)
        {
            ...
            int articlecatrgoryID = GetQueryIntValue("id");

            // 从每个选中的行中获取ID（在Grid1中定义的DataKeyNames）
            List<int> ids = GetSelectedIDsFromHiddenField(hfSelectedIDS);


            ArticleCategory articlecatrgory = Attach<ArticleCategory>(articlecatrgoryID);

            DB.Articles.Where(a => ids.Contains(a.ID))
                .ToList()
                .ForEach(a => a.ArticleCategory = articlecatrgory);
